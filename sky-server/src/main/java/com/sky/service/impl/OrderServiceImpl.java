...
//        );
    ...
    public PageResult adminList(OrdersPageQueryDTO ordersPageQueryDTO) {
        ...
        Page<Orders> orders = orderMapper.adminList(ordersPageQueryDTO);
        //
        List<OrderVO> list = new ArrayList<OrderVO>();
        if(orders!=null && orders.getTotal() > 0) {
            for (Orders item : orders) {
                ...
                for (OrderDetail orderDetail : ordersDetailList) {
                    String name = orderDetail.getName();
                    int number = orderDetail.getNumber();
                    orderName.append(name).append("*").append(number).append(";");
                }
                orderVO.setOrderDishes(orderName.toString());
                list.add(orderVO);
            }
        }
        return new PageResult(orders.getTotal(),list);
    ...
    public OrderStatisticsVO getStatistics() {
        ...
        Integer toBeConfirmed = orderMapper.listStatistics(Orders.TO_BE_CONFIRMED);

       Integer confirmed = orderMapper.listStatistics(Orders.CONFIRMED);



      Integer deliveryInProgress = orderMapper.listStatistics(Orders.DELIVERY_IN_PROGRESS);


      orderStatistics.setToBeConfirmed(toBeConfirmed);
      orderStatistics.setConfirmed(confirmed);
      orderStatistics.setDeliveryInProgress(deliveryInProgress);
